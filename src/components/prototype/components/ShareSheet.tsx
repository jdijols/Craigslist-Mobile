    ...
    <AnimatePresence>
      {open && (
        <div className="absolute inset-0 z-50" style={{ pointerEvents: "auto" }}>
          ...
          <motion.div
            key="share-sheet"
            className="absolute bottom-0 left-0 right-0 z-50 flex flex-col rounded-t-[14px] bg-cl-bg-secondary shadow-xl"
            style={{ maxHeight: "88%" }}
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
          ...
          >
            ...
            <div className="flex items-center gap-3 px-4 pt-4 pb-3">
              {variant === "search" ? (
                <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-cl-surface shadow-sm">
                  <img
                ...
                <div className="h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-cl-border shadow-sm">
                  {image ? (
                    <img src={image} alt="" className="h-full w-full object-cover" />
                  ) : (
                    <div className="h-full w-full bg-cl-border" />
                  )}
                </div>
              )}
              <div className="min-w-0 flex-1">
                <p className="truncate text-[13px] font-semibold text-cl-text">
                  {title}
                </p>
                <p className="text-[11px] text-cl-text-muted">
              ...
              <button
                type="button"
                onClick={onClose}
                className="flex h-11 w-11 min-h-[44px] min-w-[44px] shrink-0 items-center justify-center rounded-full bg-cl-border outline-none active:bg-cl-active"
                aria-label="Close"
              >
                <X className="h-4 w-4 text-cl-text-muted" strokeWidth={2.5} />
              </button>
            </div>
            ...
            <div className="flex gap-4 overflow-x-auto px-5 py-3 scrollbar-none">
              {CONTACTS.map((c) => (
                <div key={c.label} className="flex w-[68px] shrink-0 flex-col items-center gap-1.5">
                  ...
                  <span className="w-full truncate text-center text-[10px] text-cl-text">
                    {c.label}
                  </span>
                </div>
              ))}
            ...
            <div className="flex gap-4 overflow-x-auto px-5 py-3 scrollbar-none">
              {ACTIONS.map((action) => (
                <div key={action.label} className="flex w-[68px] shrink-0 flex-col items-center gap-1.5">
                  <div className="flex h-[52px] w-[52px] items-center justify-center rounded-[12px] bg-cl-surface shadow-sm">
            ...
            <div className="flex justify-center pb-8 pt-2">
              <div className="rounded-full border border-cl-border bg-cl-surface px-5 py-1.5 shadow-sm">
                <span className="text-[13px] font-medium text-cl-text">
                  Edit Actions
                </span>
              </div>
            </div>
...
function Divider() {
  return <div className="mx-4 border-t border-cl-border" />;
}
...
function ListAction({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <div className="flex min-h-[44px] items-center gap-3 px-1 py-2">
      ...
      <span className="text-[15px] text-cl-text">{label}</span>
    </div>
  );
}
...
function ReadingListIcon() {
  return (
    <svg viewBox="0 0 20 20" className="h-5 w-5 text-cl-text-muted" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" aria-hidden>
...
function ActionPlaceholder({ label }: { label: string }) {
  switch (label) {
    case "Copy":
      return (
        <svg viewBox="0 0 20 20" className="h-5 w-5 text-cl-text-muted" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
    ...
    case "View Less":
      return (
        <svg viewBox="0 0 20 20" className="h-5 w-5 text-cl-text-muted" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
          <polyline points="6 12 10 8 14 12" />
        </svg>
      );
    default:
      return <div className="h-4 w-4 rounded bg-cl-border" />;
  }
}
